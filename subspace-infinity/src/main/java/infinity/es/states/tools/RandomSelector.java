 ...
 */
import static infinity.es.states.tools.Preconditions.checkArgument;
 ...
 * @param <T> the type to base this random selector on
 */
@ThreadSafe
public final class RandomSelector<T> {
   ...
   * @param <T> the collection type
   * @param elements the collection elements
   * @param weighter the weight distribution 
   * @return a weighted random selector
   ...
   *
   * @param random a random to base the next element on
   * @return the next randomly selected element
   */
   ...
   * environments.
   *
   * @param random a random to create a stream with
   * @return the stream
   */
