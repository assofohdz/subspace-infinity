...
- **Client-side `EventType` orphans.** [`ClientEvent`](../infinity-client/src/main/java/infinity/client/ClientEvent.java) declares 4 EventType constants (`clientConnected`, `sessionStarted`, `sessionEnded`, `clientDisconnected`) with zero `EventBus.publish` sites in the tree; `MainMenuState` adds listeners that will never fire. P2-j retired the server-tier orphans but didn't sweep the client tier. Decide per constant: wire a publisher (likely in `ConnectionState` / session host bridge) or delete the constant + its listeners. *Source: 2026-05-14 cross-cutting re-verification.*
...
- **Naming hazards** flagged in the audit were three: `StatsSystem` (telemetry, not ADR-0001 `*Stats`), api-side `infinity.sim` vs server-side `infinity.sim`, and "config" pre-disambiguation between CCP templates and the settings pipeline. All three resolved: P2-b renamed `StatsSystem` → `ServerTelemetrySystem`; commit `6cffd4b9` relocated server impls to `infinity.sim.internal..` and `LayerDependencyTest` Rule 1's `resideOutsideOfPackage("infinity.sim.internal..")` clause now catches cross-tier leaks at that boundary; the third is documented in CONTEXT.md.

- **The bus was severely underused.** Three `EventBus.publish` sites in the whole codebase pre-P2-j; six declared `EventType` constants never published; an orphan `infinity.sim.Events` enum paralleling the bus surface. P2-j retired the server-side orphans; current server tier is lean (3 publish sites = `playerLoggedOn`/`playerLoggedOff`/`shipSpawned`, all live). **Client tier still has 4 orphan `EventType` constants in [`ClientEvent`](../infinity-client/src/main/java/infinity/client/ClientEvent.java) (`clientConnected`, `sessionStarted`, `sessionEnded`, `clientDisconnected`) — listeners attached in `MainMenuState`, zero publishers in the tree.** The "any new event must ship with a publisher in the same PR" discipline applies going forward; the client orphans want a separate sweep (see follow-ups).
