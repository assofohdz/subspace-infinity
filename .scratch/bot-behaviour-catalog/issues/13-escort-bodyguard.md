...
[Bot behaviour catalog PRD](../PRD.md) — behaviour 13 of 30. Catalogued in [ADR-0016 §Team / support](../../../docs/adr/0016-bot-behaviour-catalog.md). Phase 2.

## What to build

Register the `escort / bodyguard` `Behaviour` ([ADR-0013](../../../docs/adr/0013-bot-tactical-goal-layer.md)) with its `synergy { }` entry ([ADR-0014](../../../docs/adr/0014-capability-derived-bot-composition.md)) and per-goal Execute Sequence. Stay near a high-value teammate, intercept threats.

## Utility spec (from ADR-0016)

- **Hard gate:** none
- **Situational fit:** `0.35 (ward is anchor/flagger/low-energy) + 0.30 (threats inbound) + 0.20 (intercept ability) + 0.15 support`
- **Capability affinity (synergy `bonus`):** mobility, repel
- **Substrate read:** ADR-0015 role + ally density

## Acceptance criteria

- [ ] `escort / bodyguard` `Behaviour` impl: `enumerate()` produces its goal candidates; `intrinsicScore()` implements the fit formula above over the ADR-0016 input vocabulary
