...
[Bot behaviour catalog PRD](../PRD.md) — behaviour 08 of 30. Catalogued in [ADR-0016 §Spatial / map control](../../../docs/adr/0016-bot-behaviour-catalog.md). Phase 1.

## What to build

Register the `area-denial` `Behaviour` ([ADR-0013](../../../docs/adr/0013-bot-tactical-goal-layer.md)) with its `synergy { }` entry ([ADR-0014](../../../docs/adr/0014-capability-derived-bot-composition.md)) and per-goal Execute Sequence. Seed mines/bombs into a high-traffic corridor.

## Utility spec (from ADR-0016)

- **Hard gate:** `requires maxMines>0 ∨ bombs`
- **Situational fit:** `0.35 traversal_rate + 0.30 mine_headroom + 0.20 position_value + 0.15 (choke strength)`
- **Capability affinity (synergy `bonus`):** maxMines, areaDamage
- **Substrate read:** CombatDensityField + chokepoint list

## Acceptance criteria

- [ ] `area-denial` `Behaviour` impl: `enumerate()` produces its goal candidates; `intrinsicScore()` implements the fit formula above over the ADR-0016 input vocabulary
