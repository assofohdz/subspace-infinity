...
[Bot behaviour catalog PRD](../PRD.md) — behaviour 11 of 30. Catalogued in [ADR-0016 §Spatial / map control](../../../docs/adr/0016-bot-behaviour-catalog.md). Phase 2.

## What to build

Register the `push / advance` `Behaviour` ([ADR-0013](../../../docs/adr/0013-bot-tactical-goal-layer.md)) with its `synergy { }` entry ([ADR-0014](../../../docs/adr/0014-capability-derived-bot-composition.md)) and per-goal Execute Sequence. Move the team frontline forward.

## Utility spec (from ADR-0016)

- **Hard gate:** none
- **Situational fit:** `0.35 numerical_adv + 0.25 support + 0.20 mine_headroom(ahead) + 0.20 (recent-kill momentum)`
- **Capability affinity (synergy `bonus`):** mobility, tankiness
- **Substrate read:** ally/enemy density + ArenaObjective

## Acceptance criteria

- [ ] `push / advance` `Behaviour` impl: `enumerate()` produces its goal candidates; `intrinsicScore()` implements the fit formula above over the ADR-0016 input vocabulary
