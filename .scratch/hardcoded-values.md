...
| ~~[ContactSystem.java](../infinity/src/main/java/infinity/systems/ContactSystem.java) `contact.friction = 0.0` (ship-vs-static branch)~~ | ~~Per-contact tangential-velocity drain on ship-vs-wall hits~~ | ~~`0.0`~~ | **Resolved** — promoted to `ArenaConfig.wallFriction` at arena scope (map feel, not ship feel). DSL: `wallFriction N` inside `arena { … }`, validated to `[0, 1]`. Implementation note: `contact.friction` itself stays `0.0` because the resolver applies friction as an impulse at the contact point — at an off-center sphere contact that creates `r × impulse` torque that rotates ship heading toward the wall (wrong for arcade ships). Instead, `ContactSystem` decomposes `body1.linearVelocity` against `contact.contactNormal` and damps the tangential component by `wallFriction` directly. Keyed off the body's `ArenaId`; default `0.0` for bodies without one. |
| ~~[FlatTileBlockFactory.java](../infinity/src/main/java/infinity/client/view/FlatTileBlockFactory.java) `TILE_PIXELS`~~ | ~~Inner tile size in atlas pixels~~ | ~~`16`~~ | **Resolved — format constant.** Subspace BMP layout (16x16 tiles in 19x10 grid); not a tuning knob. Stays in Java. |
| ~~[FlatTileBlockFactory.java](../infinity/src/main/java/infinity/client/view/FlatTileBlockFactory.java) `GUTTER_PIXELS`~~ | ~~Replicate-padding around each tile in the loaded atlas~~ | ~~`1`~~ | **Resolved — filtering-correctness constant.** 1 px clears trilinear/anisotropic 4-texel kernels; bumping it would only fix a different filter kernel, not tune a feel. Stays in Java. |
...
| ~~[RadarState.java](../infinity/src/main/java/infinity/client/states/RadarState.java) `RADAR_PIXEL_SIZE`~~ | ~~Off-screen radar texture + GUI quad size in px~~ | ~~`218`~~ | **Resolved** — promoted to `RadarTheme.pixelSize()`. Default unchanged. |
| ~~[RadarState.java](../infinity/src/main/java/infinity/client/states/RadarState.java) `RADAR_CAM_HEIGHT`~~ | ~~Top-down ortho-cam Y above the avatar~~ | ~~`1000f`~~ | **Resolved — internal projection plumbing.** Parallel projection: height doesn't change rendered size, only the depth slab. No tuning value. Stays in Java. |
| ~~[RadarState.java](../infinity/src/main/java/infinity/client/states/RadarState.java) `RADAR_CAM_NEAR` / `RADAR_CAM_FAR`~~ | ~~Ortho-cam depth slab~~ | ~~`1f` / `5000f`~~ | **Resolved — internal projection plumbing.** Pairs with `RADAR_CAM_HEIGHT`; just needs to swallow any expected object Y. Stays in Java. |
| ~~[RadarState.java](../infinity/src/main/java/infinity/client/states/RadarState.java) `DEFAULT_RANGE_WORLD_UNITS`~~ | ~~Frustum half-extent before `RadarRange` resolves~~ | ~~`256.0`~~ | **Resolved — bridge value.** Used for the first frame(s) until the avatar's `RadarRange` arrives. Per-ship value already lives in `ShipConfig.radarRange`. Stays in Java as a divide-by-zero guard. |
| ~~[RadarState.java](../infinity/src/main/java/infinity/client/states/RadarState.java) `RADAR_CANONICAL_RANGE`~~ | ~~Reference radar range at which `RadarBlipFactory` mesh sizes render at their nominal pixel size~~ | ~~`256.0`~~ | **Resolved** — promoted to `RadarTheme.canonicalRangeWorldUnits()`. Default unchanged. |
| ~~[RadarState.java](../infinity/src/main/java/infinity/client/states/RadarState.java) `bp.initialize(avatarEntityId, 12)`~~ | ~~`BodyPosition` ring-buffer history depth~~ | ~~`12`~~ | **Resolved — SimEthereal-side convention.** Mirrors the same `12` in `AvatarMovementState.getInterpolatedAvatarPosition`; if upstream changes, both follow. Stays in Java. |
...
| ~~[RadarBlipFactory.java](../infinity/src/main/java/infinity/client/view/RadarBlipFactory.java) `SHIP_DOT_RADIUS` / `DOT_SEGMENTS` / `STATIC_BLIP_HALF_SIZE`~~ | ~~Ship-blip disc radius / fan smoothness / static-blip half-extent (world units)~~ | ~~`5f` / `16` / `10f`~~ | **Resolved** — promoted to `RadarTheme.{shipDotRadius, dotSegments, staticBlipHalfSize}()`; passed into `RadarBlipFactory` via constructor. Defaults unchanged. |
