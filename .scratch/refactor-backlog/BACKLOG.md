...
- ~~**Damage** — `WeaponsSystem.{BULLET,BOMB,GRAVBOMB}_DAMAGE` + `ActionSystem.THOR_DAMAGE`~~ — promoted.
- ~~**Projectile decays** — `WeaponsSystem.{BULLET,GRAVBOMB,MINE}_DECAY_MS` + `ActionSystem.THOR_DECAY_MS`~~ — promoted.
...
- ~~**Bomb/Gun/Mine first-time-acquisition reads `ShipConfig` at apply time.**~~ Fixed. The first-time-acquisition branch in [`BombPrizeApplier`](../../infinity/src/main/java/infinity/systems/prize/BombPrizeApplier.java) / [`GunPrizeApplier`](../../infinity/src/main/java/infinity/systems/prize/GunPrizeApplier.java) / [`MinePrizeApplier`](../../infinity/src/main/java/infinity/systems/prize/MinePrizeApplier.java) is gone — the `BombMaxLevel != null && BombCurrentLevel == null` combination is unreachable in current spawn flow (both project together on respawn), so the applier now logs a warning if it ever surfaces and skips. `getShipConfig()` and the `ConfigRegistrySystem` field were dropped from [`PrizeApplierContext`](../../infinity/src/main/java/infinity/systems/prize/PrizeApplierContext.java); appliers are pure component reads. If a real "ship has Max but no Current" scenario emerges, the proper fix is projecting `*StartLevel` components at spawn, but that's deferred until evidence surfaces.
