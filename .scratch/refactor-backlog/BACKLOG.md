...
### `BitMap` decoupling from Swing

[`BitMap.java:13`](../../infinity/src/main/java/infinity/map/BitMap.java) declares `class BitMap extends JPanel` so it can call inherited `Component.createImage(MemoryImageSource)` to materialize the BMP tileset embedded in `.lvl` files. A BMP decoder has no business being a Swing component: it's never added to a container, never rendered, and the `JPanel` superclass exists solely as a back door to `createImage`. The same pattern leaks into `BitmapSaving.java` (uses `PixelGrabber`/`ImageObserver`).

The cost shows up on macOS. Instantiating any AWT/Swing component on macOS triggers AppKit peer initialization, which wants the JVM main thread — the same thread GLFW already owns via `-XstartOnFirstThread`. The two deadlock during arena auto-load (synchronous portion of `MapSystem.loadMap` never returns past the "Loading map" log). The current workaround is `-Djava.awt.headless=true` baked into the [`runMac`](../../infinity/build.gradle) Gradle task; in headless mode AWT skips peer creation, so `JPanel` instantiation is benign. It's a hack — every macOS dev pays for a structural choice that nothing in `BitMap` actually needs.

Fix shape: replace the `createImage(MemoryImageSource)` calls with direct pixel-buffer construction (return `int[]` or a small POJO holding `width × height × pixels`), and have the consumer (`LevelLoader` / `MapState`) build a JME `com.jme3.texture.Image` from the buffer rather than going through `java.awt.Image`. Drop `extends JPanel`. Same treatment for `BitmapSaving`. Once both are AWT-free, remove `-Djava.awt.headless=true` from `runMac` (and possibly fold `runMac` into the default `run` if no AWT remains anywhere on the load path).
