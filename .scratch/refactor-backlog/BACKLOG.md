...
- **Delete `SimpleFileFilter`.** [`infinity/util/SimpleFileFilter.java`](../../infinity/src/main/java/infinity/util/SimpleFileFilter.java) — 163-line Apache JServ-vintage `FileFilter` with **zero callers** anywhere in the codebase. Confirmed orphan; safe to delete outright. Pairs naturally with the next `Preconditions`-style audit pass.

- **Delete `SettingsTypes`.** [`infinity/settings/SettingsTypes.java`](../../infinity/src/main/java/infinity/settings/SettingsTypes.java) — 489-line catalog of `private static String` constants for INI key names (e.g. `BOMB_BOMBDAMAGELEVEL = "Bomb-BombDamageLevel"`). **Zero callers** — no other class references `SettingsTypes` or any of its constants. Verified orphan; safe to delete outright. The actual settings access path goes through `SettingsSystem` reading `Ini` directly, not through this catalog.
