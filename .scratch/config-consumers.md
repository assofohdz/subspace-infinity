...
| `BulletConfig.damage` | `WeaponsSystem.createProjectileGun()` | `template` via `ConfigRegistry.weapons().bullet()` | Phase B-wired: `GroovyWeaponsLoader.load()` reads `[Bullet] BulletDamageLevel` from the merged Groovy fragment store at arena-load and on fragment hot-reload; `ArenaSystem.applyWeaponsConfig` folds the resulting `WeaponsConfig` into the arena's `ConfigRegistry` snapshot. Missing key falls back to `BulletConfig.DEFAULTS` (svs preset baseline = 100). |
