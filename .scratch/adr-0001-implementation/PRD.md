...
- ✅ **Brick** — `Brick` (count, kept) + `BrickStats(max)` + `BrickChange`; `BrickSystem` drains; migrated `BrickPrizeApplier` to emit Change holder; `ShipWeaponsProjector` writes Continuous + Stats directly (factory tier); `ConsumableSystem.deductCostOfActionBrick` emits `BrickChange(-1)`. `BrickMax` deleted. Brick wall-length (`BrickSpan`) stays on the marker entity from `MapFactory` (per-arena, not per-ship).
...
- ✅ **Decoy** — `Decoy` (count, kept) + `DecoyStats(max)` + `DecoyChange`; `DecoySystem` drains; migrated `DecoyPrizeApplier` to emit Change holder; `ShipWeaponsProjector` writes Continuous + Stats directly (factory tier); `ConsumableSystem.deductCostOfActionDecoy` emits `DecoyChange(-1)`. `DecoyMax` deleted.
...
- ✅ **Portal** — `Portal` (count, kept) + `PortalStats(max)` + `PortalChange`; `PortalSystem` drains; migrated `PortalPrizeApplier` to emit Change holder; `ShipWeaponsProjector` writes Continuous + Stats directly (factory tier); `ConsumableSystem.deductCostOfActionPortal` emits `PortalChange(-1)`. `PortalMax` deleted.
- ✅ **Repel** — `Repel` (count, kept) + `RepelStats(max)` + `RepelChange`; `RepelCountSystem` drains; migrated `RepelPrizeApplier` to emit Change holder; `ShipWeaponsProjector` writes Continuous + Stats directly (factory tier); `ConsumableSystem.deductCostOfActionRepel` emits `RepelChange(-1)`. `RepelMax` deleted. Sibling `RepelSystem` continues to own the impulse mechanic — option (b) split because the impulse system depends on `MPhysSystem` registered after `DecaySystem`; see `RepelCountSystem` Javadoc. `RepelDistance`/`RepelSpeed` stay arena-tier marker stamps.
- ✅ **Rocket** — `Rocket` (count, kept) + `RocketStats(max, buffDurationMillis)` + `RocketChange`; `RocketSystem` drains; migrated `RocketPrizeApplier` to emit Change holder; `ShipWeaponsProjector` writes Continuous + Stats directly (factory tier); `ConsumableSystem.deductCostOfActionRocket` emits `RocketChange(-1)`. `RocketMax` and `RocketTime` deleted (buff duration absorbed into `RocketStats.buffDurationMillis`).
...
- ✅ **Thor** — `ThorCurrentCount` (kept) + `ThorStats(max, fireDelayMillis)` + `ThorChange`; `ThorSystem` drains; migrated `ThorPrizeApplier` to emit Change holder + fixed the `ThorFireDelay` fallback divergence (was `new ThorFireDelay(1000)` clobbering spawn-projected per-ship value; now seeds from `ThorStats.fireDelayMillis()` only on first-time acquire when no spawn-projected timer exists); `ShipWeaponsProjector` writes Continuous + Stats + initial `ThorFireDelay` directly (factory tier); `ConsumableSystem.deductCostOfActionThor` emits `ThorChange(-1)`; `ConsumableSystem.setCoolDownThor` re-stamps `ThorFireDelay` from `ThorStats.fireDelayMillis()` rather than `gfd.copy()`. `ThorMaxCount` deleted.
