...
- **`BotBrainSystem`** — `MovementInput` on `BotShip` entities (per-tick brain-tick result; the `AIEntities.createMobShip` factory-tier seed at spawn is the disjoint-entity-set shape per ADR-0009, not a competing runtime writer); `BotDebug` (per-tick wire-crossing snapshot of branch / target / intent / clock-hour-to-target; clients observe via SimEthereal sync for the bot-debug HUD); `BotCapability` (server-only; written in `refreshDerivation` only when the arena `ConfigRegistry` reference changes — registry-change-edge, not per-tick; carries the projected `CapabilityProfile` for the bot's ship type per ADR-0014); `BotRole` (server-only; first-tick assignment from the arena objective then fixed for the entity's lifetime per ADR-0015 + bot-ai-v3 #01.3 — event-driven reassignment deferred to v2.x); `FlowFieldDebug` on `PlayerShip` entities (dev-only, gated by `ZoneBotAiConfig.flowFieldDebug`; per-density-cadence wire-crossing snapshot of the blended bot-steering flow patch around each player for the client flow-field overlay — disjoint entity set from the bot-side `MovementInput`/`BotDebug` writes).
