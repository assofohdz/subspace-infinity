...
| [ContactSystem.java](../infinity/src/main/java/infinity/systems/ContactSystem.java) `contact.friction = 0.0` (ship-vs-static branch) | Per-contact tangential-friction coefficient for ship-vs-wall hits | `0.0` | Tuning knob. Different ships could plausibly want different wall friction (slidey vs grippy). Promote to per-ship `ShipConfig.wallFriction()` → `WallFriction` component (parallel to `BounceRestitution` from Pattern 4 #4). Follow-up #13 in [`pattern4-followups.md`](pattern4-followups.md). |
