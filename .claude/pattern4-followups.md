...
## 4. Tuning constants that probably belong in `ShipStat` / Groovy

Currently global Java constants. Once different ships need different feel, promote each to a per-ship Groovy field:

- `PlayerDriver.DRAG_FACTOR = 0.10`
- `PlayerDriver.TURN_RESPONSIVENESS = 8.0`
- `ContactSystem` `contact.restitution = 1` (perfect elastic bounce — may want 0.75 for energy-loss-on-wall)
