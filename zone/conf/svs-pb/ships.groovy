// PowerBall ship settings — applied uniformly to all 8 ships. Replaces the
// INI ship-all + composite svs.conf shape (which used [Warbird] #include
// ship-all; [Javelin] #include ship-all; …) with the GroovyFragmentLoader
// shipSections splat.

shipSections('Warbird', 'Javelin', 'Spider', 'Leviathan',
             'Terrier', 'Weasel', 'Lancaster', 'Shark') {
    SuperTime 3000
    ShieldsTime 4000
    Gravity 1500
    GravityTopSpeed 100
    BulletFireEnergy 20
    MultiFireEnergy 30
    BombFireEnergy 300
    BombFireEnergyUpgrade 50
    LandmineFireEnergy 300
    LandmineFireEnergyUpgrade 150
    BulletSpeed 2000
    BombSpeed 2000
    BulletFireDelay 30
    MultiFireDelay 50
    BombFireDelay 150
    LandmineFireDelay 125
    MultiFireAngle 500
    CloakEnergy 100
    StealthEnergy 333
    AntiWarpEnergy 100
    XRadarEnergy 250
    MaximumRotation 230
    MaximumThrust 17
    MaximumSpeed 3250
    MaximumRecharge 1150
    MaximumEnergy 1300
    InitialRotation 230
    InitialThrust 17
    InitialSpeed 3250
    InitialRecharge 1150
    InitialEnergy 1300
    UpgradeRotation 40
    UpgradeThrust 2
    UpgradeSpeed 250
    UpgradeRecharge 166
    UpgradeEnergy 100
    AfterburnerEnergy 1200
    BombThrust 400
    BurstSpeed 3000
    BurstShrapnel 24
    TurretThrustPenalty 1
    TurretSpeedPenalty 125
    TurretLimit 0
    RocketTime 300
    InitialBounty 100
    MaxMines 1
    RepelMax 0
    BurstMax 1
    DecoyMax 0
    ThorMax 1
    BrickMax 1
    PortalMax 3
    RocketMax 3
    InitialRepel 0
    InitialBurst 0
    InitialBrick 0
    InitialRocket 0
    InitialThor 0
    InitialDecoy 0
    InitialPortal 0
    BombBounceCount 0
    CloakStatus 0
    StealthStatus 0
    XRadarStatus 0
    AntiWarpStatus 0
    DamageFactor 0
    PrizeShareLimit 100
    AttachBounty 40
    ShrapnelMax 6
    ShrapnelRate 1
    MaxGuns 3
    MaxBombs 2
    InitialGuns 3
    InitialBombs 2
    EmpBomb 0
    SeeMines 0
    DoubleBarrel 0
}
