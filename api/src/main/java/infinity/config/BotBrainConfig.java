 ...
 * second {@code BrainArchetype}. Live-reloaded via {@code GroovyBotBrainLoader} +
 * {@code bot-tuning.groovy} fragment; consumers should read this template only at
 * blackboard-construction time, never on the per-tick hot path (per
 * Config-Component Projection, ADR-0002).
 *
...
public record BotBrainConfig(
  ...
  public static final BotBrainConfig DEFAULTS =
      new BotBrainConfig("Brawler", 30.0, 20.0, 15.0, 0.6, 0.5, 15.0);

  public BotBrainConfig() {
    this("Brawler", 30.0, 20.0, 15.0, 0.6, 0.5, 15.0);
  }
}
