...
- [`multi-machine-workflow.md`](.claude/rules/multi-machine-workflow.md) — branch-per-slice, push-at-every-commit, `git pull --rebase` at session start; coordinate via [`.scratch/active-work.md`](.scratch/active-work.md); machine identity stored per-machine in auto-memory (`machine_identity.md`). Bootstrap on a new machine: ask the user which machine you're on, then write the memory file.
