...
- [`lsp-for-java-navigation.md`](.claude/rules/lsp-for-java-navigation.md) — prefer `mcp__java-lsp__*` (and the generic `LSP` tool for hover / impls / call hierarchy) over `grep` for Java navigation; type-aware lookups beat text search on a ~100k-LOC codebase (`api/src/**/*.java`, `infinity-server/src/**/*.java`, `infinity-client/src/**/*.java`).
