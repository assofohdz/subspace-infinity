...
/** Minimal drain pin for {@link AntiwarpSystem} — same shape as {@link CloakSystemChangeDrainTest}. */
public class AntiwarpSystemChangeDrainTest {
  ...
  public void oneShotChange_flipsToggleAndReapsHolder() {
    ...
    try {
      ...
      f.ed.setComponent(ship, new AntiwarpActive(false));
      final EntityId h = f.ed.createEntity();
      f.ed.setComponents(h, ChangeTarget.self(ship), new AntiwarpActiveChange(true));

      f.systems.update();

      assertTrue(f.ed.getComponent(ship, AntiwarpActive.class).isActive());
      assertNull(f.ed.getComponent(h, AntiwarpActiveChange.class));
    } finally {
      f.shutdown();
    }
  }
  ...
  public void temporaryChangeReversedOnRemove() {
    ...
    try {
      ...
      final EntityId h = f.ed.createEntity();
      f.ed.setComponents(
          h, ChangeTarget.self(ship), new AntiwarpActiveChange(true), new Decay(0L, 1_000_000L));
      ...
      assertFalse(f.ed.getComponent(ship, AntiwarpActive.class).isActive());
    } finally {
      f.shutdown();
    }
  }
