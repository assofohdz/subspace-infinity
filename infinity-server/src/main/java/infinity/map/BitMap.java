...
public class BitMap {
    ...
    public void readBitMap(final boolean trans) {
        ...
        width = array.getInt(4);
        height = array.getInt(8);
        bitCount = array.getShort(14);
        final int compressionType = array.getInt(16);
        ...
        if (compressionType == BI_RGB && bitCount <= 8) {
            readInRGB();
        } else if (compressionType == BI_RLE8 && bitCount == 8) {
            readInRLE8();
        } else if (compressionType == BI_RGB && bitCount == 24) {
            readInRGB24(trans);
        }
    }
     ...
     * was detected (and {@link #size} / {@link #eLvlOffset} are populated),
    ...
    /**
     * Read the 4-byte palette entries from the stream into {@link #colorTable}
     * (paletted-BMP path only; {@code bitCount &le; 8}). Each entry is
    ...
    private void readColorTable(final boolean trans) {
        colorTable = new int[(int) Math.pow(2, bitCount)];
        final int colorsUsed = (int) Math.pow(2, bitCount);
        for (int i = 0; i < colorsUsed; i++) {
            final byte[] c = readIn(4);
            final ByteBuffer array = LvlBinUtil.wrapLE(c);
            colorTable[i] = (array.getInt(0) & 0xffffff) + 0xff000000;
            if (colorTable[i] == 0xff000000 && trans) {
                colorTable[i] = colorTable[i] & 0x00000000;
            }
        }
    }
    ...
    public void readInRGB() {
        ...
        final int shift[] = new int[8 / bitCount];
        for (int i = 0; i < 8 / bitCount; i++) {
            shift[i] = 8 - ((i + 1) * bitCount);
        }
        ...
        int pad = 4 - (int) Math.ceil(width * bitCount / 8.0) % 4;
        if (pad == 4) {
            pad = 0;
        }
        ...
        for (int i = 0; i < height * width; i++) {
            image[y * width + x] = colorTable[a >> shift[bit] & mask];
    ...
    public void readInRLE8() {
        ...
        while (a != 0 || b != 1) {
            if (a == 0) {
                ...
                for (int i = 0; i < a; i++) {
                    image[y * width + x] = colorTable[paletteIndex];
                    x++;
                }
            }
            a = readByte();
     ...
     * {@link #image} for absolute-run escapes ({@code b >= 3}) and returns
    ...
    private int[] decodeRleEscape(final int b, final int yIn, final int xIn) {
        ...
        if (b == 0) {
            ...
            for (int i = 0; i < b; i++) {
                image[y * width + x] = colorTable[readByte()];
                x++;
            }
     ...
     * 8-bit paths: {@code image} is top-to-bottom ARGB where index 0 is the top-left pixel.
     *
    ...
    public void readInRGB24(final boolean trans) {
        ...
        for (int y = height - 1; y >= 0; y--) {
            for (int x = 0; x < width; x++) {
                ...
                image[y * width + x] = (alpha << 24) | (r << 16) | (g << 8) | b;
            }
            for (int j = 0; j < pad; j++) {
                readByte();
            }
    ...
    public BitmapData getBitmap() {
        return new BitmapData(width, height, image);
    }
