...
/** Canonical writer for {@link AntiwarpActive}; drains {@link AntiwarpActiveChange}. See ADR 0001 + {@link CloakSystem}. */
public class AntiwarpSystem extends BaseInfinitySystem {
  ...
  protected void initialize() {
    ...
    changes = ed.getEntities(AntiwarpActiveChange.class, ChangeTarget.class);
  }
  ...
  private void drainChanges() {
    ...
    for (final Entity added : changes.getAddedEntities()) {
      ...
      final boolean newValue = added.get(AntiwarpActiveChange.class).newValue();
  ...
  private boolean applyValue(final EntityId target, final boolean newValue) {
    final AntiwarpActive current = ed.getComponent(target, AntiwarpActive.class);
    final boolean previous = current != null && current.isActive();
    if (previous == newValue) {
      return previous;
    }
    ed.setComponent(target, new AntiwarpActive(newValue));
    return previous;
  }
}
