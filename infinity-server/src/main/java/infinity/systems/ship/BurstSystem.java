...
/** Canonical writer for live {@link Burst} (count); drains {@link BurstChange}, clamping at {@link BurstStats#max}. See ADR 0001 + {@link ThrustSystem}. */
public class BurstSystem extends BaseInfinitySystem {
  ...
  protected void initialize() {
    ...
    changes = ed.getEntities(BurstChange.class, ChangeTarget.class);
  }
  ...
  private void drainChanges() {
    ...
    for (final Entity added : changes.getAddedEntities()) {
      ...
      final int delta = added.get(BurstChange.class).delta();
  ...
  private void applyDelta(final EntityId target, final int delta) {
    final BurstStats stats = ed.getComponent(target, BurstStats.class);
    if (stats == null || stats.max() <= 0) {
      return; // ship not allowed bursts
    }
    final Burst current = ed.getComponent(target, Burst.class);
    ...
    ed.setComponent(target, new Burst(clamped));
  }
}
