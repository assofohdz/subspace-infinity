...
/** Canonical writer for {@link StealthActive}; drains {@link StealthActiveChange}. See ADR 0001 + {@link CloakSystem}. */
public class StealthSystem extends BaseInfinitySystem {
  ...
  protected void initialize() {
    ...
    changes = ed.getEntities(StealthActiveChange.class, ChangeTarget.class);
  }
  ...
  private void drainChanges() {
    ...
    for (final Entity added : changes.getAddedEntities()) {
      ...
      final boolean newValue = added.get(StealthActiveChange.class).newValue();
  ...
  private boolean applyValue(final EntityId target, final boolean newValue) {
    final StealthActive current = ed.getComponent(target, StealthActive.class);
    final boolean previous = current != null && current.isActive();
    if (previous == newValue) {
      return previous;
    }
    ed.setComponent(target, new StealthActive(newValue));
    return previous;
  }
}
