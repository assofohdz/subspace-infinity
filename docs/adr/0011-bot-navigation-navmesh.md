...
**Status:** Accepted (revised 2026-05-23) — implemented (door-tile invalidation + tile-supersampling deferred)
**Implementation:** Slice #02 (db90765e) landed the flow-field substrate (`DistanceField`/`GradientField`/`NavigationFields` + Dijkstra builder + tracer). Slice #03 (1080a9aa) landed production nav: `AsyncNavigationFields` (worker-thread per-goal Dijkstra, `getNow(EMPTY)` non-blocking reads, TTL + LRU eviction, static `pin()`, goal-snap), the `MapSystem` passability seam (`derivePassability`, projector axis-flip; **raw** passability — clearance erosion reverted), plus the first nav-consuming steering (`SteerApproachTarget` + `HasLineOfSight` gate) and `WallRepulsion` corner-escape. Door invalidation is coarse (`evictAll` on any door change); per-door-tile tracking and tile-supersampling remain deferred. Static-goal registration (`pin`) is wired but has no producers until #06/#07.
**Date:** 2026-05-23
