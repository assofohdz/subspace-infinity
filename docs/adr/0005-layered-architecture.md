...
- **Close the CubeFactory leak** by relocating server-side `infinity.sim` classes to `infinity.sim.internal..` (architectural review P0-b).
- **Extend `LayerDependencyTest`** — add `infinity.sim.internal..` to Rule 3 forbidden list after relocation; add `infinity.settings..` to Rule 1 forbidden list (architectural review P1-c).
