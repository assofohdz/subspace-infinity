...
- **Retire chicken framework.** Move current `infinity.ai.*` content → `infinity.ai.legacy.*` with `@Deprecated` on the public surface. Keep compiling during transition (it doesn't drive `BotShip`, so removal is safe) until the new stack stabilises; delete the `legacy` package once the new BT-driven bots ship and no regressions surface. Tracked separately in [`.scratch/code-todos-backlog.md`](../../.scratch/code-todos-backlog.md) as a follow-up item after the v1 lands.
- **Architectural test for `MovementInput` canonical writer on bots.** Extend [`CanonicalWriterTest`](../../infinity-server/src/test/java/infinity/architecture/CanonicalWriterTest.java) (or its sibling) to assert (a) `MovementInput` on `BotShip`-marked entities has exactly one writer: `BotBrainSystem`, and (b) no `CharacterInput` is ever stamped on a `BotShip`-marked entity (regression guard against accidentally re-introducing the NPC input shape on a ship).
- **Delete the `CharacterInput` + `UprightDriver` + `MobContainer` path** in `MovementInputSystem` once the chicken framework moves to `infinity.ai.legacy.*` and is itself deleted. `MovementInputSystem` collapses to one container (`PlayerContainer`); the NPC input shape disappears. Tracked in [`.scratch/code-todos-backlog.md`](../../.scratch/code-todos-backlog.md) as a follow-up to chicken retirement.
- **Input abstraction model — RESOLVED (2026-05-22).** Audit found that the player input pipeline is **rate-based, not target-state**: `AvatarMovementState.update()` writes `MovementInput.move = Vec3d(turnRate, 0, thrustRate)` where both components are analog values in `[-1, 1]` from Lemur's `F_TURN` / `F_THRUST` mappings; `MovementInput.facing` is **always identity** on the wire (the client never updates it) and **`PlayerDriver` ignores `facing` entirely** — it consumes only `move.x` (as rotation rate scaled by `RotationStats.max`) and `move.z` (as forward thrust). Bots emit the same shape: `BotBrainSystem` writes `MovementInput.move = Vec3d(turnRate, 0, thrustRate)`, with rates in `[-1, 1]`; the ship's per-stat caps (`RotationStats.max`, `ThrustStats.max`) translate that to physical rates in `PlayerDriver`. Reynolds steering primitives (`Pursue`, `Seek`, etc.) output exactly this shape natively — `steer(mover) → Vec3d(yaw, _, thrust)` — so each primitive's output maps directly to `MovementInput.move` with zero translation. Input-parity is now both wire-shape parity AND semantic parity: bots and players emit the same rate-shaped intent at the same abstraction level, processed by the same `PlayerDriver`, subject to the same per-ship rate caps.
- **`MovementInput.facing` field is dead.** Always identity from the client; ignored by `PlayerDriver`. Replacing `MovementInput` with a rate-only `MovementIntent(turnRate, thrustRate, flags)` would remove the dead field, but the cleanup is deferred (tracked in [`.scratch/code-todos-backlog.md`](../../.scratch/code-todos-backlog.md)).
...
- **`BotBrainConfig` Groovy fragment shape.** Settle the fragment naming (`bot-tuning.groovy` vs `brains.groovy` vs `ai.groovy`), the section grammar (per-archetype block? per-stat block?), and the adapter shape (matches existing `Groovy*Loader` pattern from [ADR-0004](./0004-settings-pipeline.md)). Decide before the v1 PRD lands; settles in implementation.
- **Strategic / team-coordination AI (GOAP layer).** Deferred. Trigger to revisit: first concrete demand for objective routing (capture-the-flag intelligence, team formation, role assignment).
- **Pathfinding.** Deferred. Trigger to revisit: first closed-corridor map design that exposes `AvoidObstacles` as insufficient.
