...
- **Single canonical writer on `CharacterInput`.** Bot inputs and human inputs use the same channel; both are clean single-writer ([ADR-0001](./0001-ecs-component-model.md)) per arena+entity scope.
...
- **Extend the existing `infinity.ai.*` chicken framework.** Rejected — the framework drives physics `RigidBody` via `MobDriver.AbstractControlDriver`; `BotShip` entities use `CharacterInput`. The two paths are not compatible, and re-targeting the chicken framework to write `CharacterInput` would touch every `Goal`/`Strategy`/`Action` site for no net architectural benefit. Cleaner to retire the chicken path and start fresh.
...
- **Layer-test enforcement:** existing `LayerDependencyTest` rules cover the new packages; no new architectural test needed for that. New `CanonicalWriterTest` entry guards `BotBrainSystem`-as-only-`CharacterInput`-writer on bot ships.
...
- **Bot input wire path.** Players write `MovementInput` via an RMI path; bots write it directly server-side from `BotBrainSystem` (no RMI round-trip needed). Decide whether to introduce an "input source" marker (`PlayerInputSource` / `BotInputSource`) for telemetry, replay, or debug; the parity guarantee holds either way. Optional, settled at implementation.
...
- [`api/src/main/java/infinity/sim/AIEntities.java`](../../api/src/main/java/infinity/sim/AIEntities.java) — current `BotShip` spawn site; the entity the new system drives. Today stamps `CharacterInput`; the v1 implementation switches it to `MovementInput`.
