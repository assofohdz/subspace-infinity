...
- **Input abstraction level — sharper input-parity audit needed.** Wire-shape parity (both bots and players write `MovementInput(move, facing, flags)`) is **not the same as semantic parity**. A keyboard player presses discrete buttons (turn-left / turn-right / thrust / fire); something *between* the keyboard and the `MovementInput` component translates those into `facing` and `move` values. The bot brain in v1 (Issue #02 tracer) writes `facing` as a direct yaw computation — bypassing whatever rate-limiting / smoothing / button-debounce logic the keyboard-to-`MovementInput` translator does on the client side. Open questions: (a) where exactly does keyboard input become `MovementInput` today (client side `PlayerMovementState`? server-side RMI handler?), (b) what abstraction does that layer operate on (button states? per-tick facing deltas? absolute facing?), (c) should the bot brain emit at *that* abstraction level (button-shaped intents, translated to `MovementInput` by the same code path the keyboard uses) rather than at the `MovementInput` level directly. Trigger to investigate: before Issue #03 (`Pursue` + `AvoidObstacles`) lands real steering — a tracer rotating in place doesn't surface the mismatch but real-time steering will. May produce a new component (`MovementIntent`?) and a translator system shared between client keyboard handling and bot brain output.
